...
@class WDExpandableToolbar;

@protocol WDExpandableToolbarDelegate <NSObject>

- (void)expandToolbar:(WDExpandableToolbar *)toolbar
 ...
 *
 ***********************************/
- (BOOL)shouldShowExpandToolbar:(WDExpandableToolbar *)expandToolbar
      whenToolbarItemDidClicked:(WDExpandableToolbarItem *)toolbarItem;
...
@property (nonatomic, weak) id<WDExpandableToolbarDelegate> delegate;


/// 最顶端的Toolbar实例 (如果本身就是最顶端，返回自己，否则返回最顶的)
@property (nonatomic, strong, readonly) WDExpandableToolbar *topToolbar;

/// 返回下一级Toolbar实例，不存在就返回nil
@property (nonatomic, weak, readonly) WDExpandableToolbar *nextLevelToolbar;
...
- (instancetype)initWithTopItems:(NSArray *)items
                  expandToolbarItems:(NSArray *)expandItems NS_DESIGNATED_INITIALIZER;
...
- (instancetype)initWithTopItems:(NSArray *)items
              nextLevelexpandToolbar:(WDExpandableToolbar *)nextLevelToolbar NS_DESIGNATED_INITIALIZER;
